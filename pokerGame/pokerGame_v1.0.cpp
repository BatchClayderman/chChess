...
public:
	...
	PlayerID getLandlord() // records
	{
		...
		case Status::Started:
		case Status::Over:
			return this->records.empty() || this->records.back().empty() ? (PlayerID)(-1) : this->records.back().back().playerID;
		case Status::Ready:
...
public:
	...
	bool interact()
	{
		...
		else
		{
			...
			if (this->playerCount ? pokerGame.initialize(this->pokerType, this->playerCount) : pokerGame.initialize(this->pokerType))
			{
				pokerGame.deal();
				pokerGame.setSortingMethod(this->sortingMethod);
				PlayerID playerID = pokerGame.getLandlord();
				if ("三人斗地主" == this->pokerType)
					pokerGame.setLandlord(true, true, true, false);
				else if ("四人斗地主" == this->pokerType)
					pokerGame.setLandlord(LandlordScore::None, LandlordScore::One, LandlordScore::Three, LandlordScore::None);
				playerID = pokerGame.getLandlord();
				for (;;)
				{
					pokerGame.display(true);
