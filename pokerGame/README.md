...
一般地，除了函数 ``processToken`` 外，所有 bool 型函数的返回值为 ``false`` 时，不会对传递为引用的参数做任何更改。
...
3) 一张牌（Card）：一张牌包含点数和花色两个字段；在日常游戏中，人们通常先念花色再念点数；在本程序中，一张牌被定义为 ``Card``，包含点数和花色两个字段。
...
1) 牌局（Game）：牌局，简称局，指一次包含开局、中局和结算等过程的完整的游戏，牌局与牌局之间通常相互独立；一般情况下，不允许玩家在牌局进行时中途离开；在日常游戏中，部分扑克牌游戏会根据上一局的输赢来决定下一局的发牌或首位出牌的玩家。
2) 牌堆（Deck）：还未派发至玩家手上的牌；牌堆通常保持不可见。
3) 牌型（Type）：根据牌局所属扑克牌游戏的规则由一张或多张扑克牌组成的牌向量（``vector``）的类型；在本程序中使用 ``TokenType`` 定义牌型，为优化复杂度，在本程序中，要不起也属于一种牌型（``TokenType::Empty``）。
4) 一手牌（Hand）：选定一张或多张扑克牌后点击一次“出牌”就能把所选的牌以一种合法的牌型一次性打出去组成的牌向量被称为一手牌；在本程序中，一手牌被定义为 ``Token``，包含玩家、牌向量和牌型三个字段，并使用牌向量的第一个元素（``cards[0]``）指示当前牌型下这手牌的大小。
5) 盖过（Cover）：如果当前一手牌能够依照所属扑克牌游戏的规则大过上一手牌，则称当前一手牌盖过上一手牌。
...
程序在识别本扑克牌时，会先捕获其英文，如果英文未能与本存储库所支持的任何扑克牌游戏匹配，则将所有数字相应地替换为中文再进行识别；以下为本存储库所支持的扑克牌游戏及其确定性规则和实现说明。
...
除牌型上允许被带的牌与母体一致外（如 66665553 被识别为飞机带小翼），其余事宜与斗地主一致。
