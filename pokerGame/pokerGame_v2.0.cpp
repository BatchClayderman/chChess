		...
		case Status::Over:
		{
			...
			if (this->status >= Status::Assigned && !this->records.empty())
			{
				...
				for (size_t round = 1; round < roundCount; ++round)
				{
					...
					{
						cout << this->cards2string(this->records[round][0].cards, "", "+", "", "要不起") << "（玩家 " << (this->records[round][0].playerID + 1) << "）";
						const size_t tokenCount = this->records[round].size();
						for (size_t tokenID = 1; tokenID < tokenCount; ++tokenID)
							cout << " -> " << this->cards2string(this->records[round][tokenID].cards, "", "+", "", "要不起") << "（玩家 " << (this->records[round][tokenID].playerID + 1) << "）";
						cout << endl;
					}
				}
				cout << endl;
...
public:
	...
	virtual bool getCurrentPlayer(PlayerID& playerID) const final
	{
		if (this->records.empty() || this->records.back().empty())
			return false;
		else if (this->records.size() >= 2)
		{
			PlayerID currentPlayerID = this->records.back().back().playerID;
			if (this->getNextPlayer(currentPlayerID))
			{
		...
		else
		{
			...
			if (this->getLastEffectivePlayer(currentPlayerID))
			{
				playerID = currentPlayerID;
				return true;
			}
	...
	{
		if (Status::Assigned == this->status && this->records.size() == 1 && !this->records[0].empty() && token.playerID < this->players.size() && token.playerID == this->records[0].back().playerID && !token.cards.empty() && (this->records[0].back().cards.size() == 1 && find(token.cards.begin(), token.cards.end(), this->records[0].back().cards[0]) != token.cards.end()) && this->processToken(token) && this->removeCards(token.cards, this->players[token.playerID]))
		{
	...
	virtual bool start(const string& description, PlayerID& playerID) final // records
	{
		if (Status::Assigned == this->status && this->records.size() == 1 && !this->records[0].empty())
		{
			Token token{};
			return this->getCurrentPlayer(token.playerID) && this->description2token(description, token) && this->start(token, playerID);
		}
		else
			return false;
	}
	virtual bool play(Token& token, PlayerID& playerID) final // records
	{
		PlayerID lastEffectivePlayerID = 0;
		if (Status::Started == this->status && this->records.size() >= 2 && !this->records.back().empty() && this->getLastEffectivePlayer(lastEffectivePlayerID) && this->processToken(token))
			if (TokenType::Empty == token.tokenType)
				if (token.playerID == playerID) // the starter for the new round
					return false;
				else
				{
					playerID = token.playerID;
	...
	virtual bool play(const string& description, const PlayerID inputPlayerID, PlayerID& outputPlayerID) final // records
	{
...
public:
	...
	bool start(Token& token, PlayerID& playerID) override final // records
	{
		PlayerID lastEffectivePlayerID = 0;
		if (Status::Assigned == this->status && this->records.size() == 1 && !this->records[0].empty() && token.playerID < this->players.size() && this->getLastEffectivePlayer(lastEffectivePlayerID) && token.playerID == lastEffectivePlayerID && !token.cards.empty() && this->processToken(token) && this->removeCards(token.cards, this->players[token.playerID]))
		{
			this->records.push_back(vector<Token>{ token });
			if (this->isOver())
			{
